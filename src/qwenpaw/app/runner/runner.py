...
from universal_entry_agent.diagnostics import elapsed_ms, get_current_trace
        ...
        **kwargs,
        ...
        if performance_trace is not None:
            performance_trace.event(
                ...
                duration_ms=elapsed_ms(approval_started),
                approval_consumed=approval_consumed,
                has_pending_response=approval_response is not None,
            )
        ...
        try:
            ...
            if self._mcp_manager is not None:
                ...
                if performance_trace is not None:
                    performance_trace.event(
                        "runner.mcp_clients",
                        duration_ms=elapsed_ms(mcp_started),
                        client_count=len(mcp_clients),
                    )
            ...
            if performance_trace is not None:
                performance_trace.event(
                    ...
                    duration_ms=elapsed_ms(config_started),
                )
            ...
            agent_create_started = time.perf_counter()
            agent = QwenPawAgent(
            ...
            if performance_trace is not None:
                performance_trace.event(
                    ...
                    duration_ms=elapsed_ms(agent_create_started),
                )
            mcp_register_started = time.perf_counter()
            await agent.register_mcp_clients()
            if performance_trace is not None:
                performance_trace.event(
                    "runner.register_mcp_clients",
                    duration_ms=elapsed_ms(mcp_register_started),
                    client_count=len(mcp_clients),
                )
            ...
            if self._chat_manager is not None:
                ...
                if performance_trace is not None:
                    performance_trace.event(
                        "runner.chat_get_or_create",
                        duration_ms=elapsed_ms(chat_started),
                        chat_id=str(chat.id),
                    )
            ...
            if performance_trace is not None:
                performance_trace.event(
                    "runner.session_load",
                    duration_ms=elapsed_ms(session_load_started),
                )
            ...
            if performance_trace is not None:
                performance_trace.event(
                    "runner.rebuild_system_prompt",
                    duration_ms=elapsed_ms(prompt_started),
                )

            if performance_trace is not None:
                performance_trace.event(
                    "runner.prepare.completed",
                    duration_ms=elapsed_ms(prepare_started),
                )
            ...
            if agent is not None and session_state_loaded:
                ...
                if performance_trace is not None:
                    performance_trace.event(
                        "runner.session_save",
                        duration_ms=elapsed_ms(session_save_started),
                    )

            if self._chat_manager is not None and chat is not None:
                chat_touch_started = time.perf_counter()
                await self._chat_manager.touch_chat(chat.id)
                if performance_trace is not None:
                    performance_trace.event(
                        "runner.chat_touch",
                        duration_ms=elapsed_ms(chat_touch_started),
                    )
            if performance_trace is not None:
                performance_trace.event(
                    "runner.total",
