...
class QwenPawAgent(ToolGuardMixin, ReActAgent):
    ...
    **must** call ``super()._acting(...)`` / ``super()._reasoning(...)``
    ...
    def __init__(
        ...
        # Register request-scoped tools supplied by an embedding application.
        # These callbacks stay outside the generic QwenPaw tool catalog and can
        # safely capture per-request/session context.
        self._register_request_tools(toolkit, namesake_strategy)
    ...
    def _register_request_tools(
        self,
        toolkit: Toolkit,
        namesake_strategy: NamesakeStrategy,
    ) -> None:
        """Register optional callable tools carried in request context."""
        business_router = self._request_context.get("business_intent_router_tool")
        if callable(business_router):
            toolkit.register_tool_function(
                business_router,
                namesake_strategy=namesake_strategy,
            )
            logger.debug("Registered request-scoped business intent router")
    ...
    async def _reasoning(
        ...
        interception active.
        """
        route_state = self._request_context.get("business_intent_route_state")
        if isinstance(route_state, dict):
            direct_response = route_state.pop("direct_response", None)
            if direct_response:
                msg = Msg(
                    name=self.name,
                    role="assistant",
                    content=[TextBlock(type="text", text=str(direct_response))],
                )
    ...
    async def reply(
        ...
        logger.info("AgentCore.reply: max_iters=%s", self.max_iters)
