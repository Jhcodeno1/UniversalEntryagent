...
class RetryChatModel(ChatModelBase):
            ...
            try:
                try:
                    ...
                    if performance_trace is not None:
                        performance_trace.event(
                            "react.llm.slot_wait",
                            duration_ms=elapsed_ms(slot_started),
                            attempt=attempt,
                ...
                if performance_trace is not None:
                    performance_trace.event(
                        ...
                        model=self.model_name,
                        response_kind=(
                            "stream" if isinstance(result, AsyncGenerator) else "complete"
                        ),
                    )
                ...
                delay = (
                    _compute_backoff(attempt, self._retry_config)
                    if will_retry
                    else 0.0
                )
                if performance_trace is not None:
    ...
    async def _wrap_stream(
        ...
        try:
            ...
            if performance_trace is not None:
                performance_trace.event(
                    "react.llm.stream_failed",
                    status="error",
                    attempt=current_attempt,
                    max_attempts=max_attempts,
                    model=self.model_name,
                    backoff_seconds=delay,
                    error=f"{failed_exc.__class__.__name__}: {failed_exc}",
                )
            logger.warning(
                    ...
                    try:
                        ...
                        if performance_trace is not None:
                            performance_trace.event(
                                ...
                                error=(
                                    f"{retry_failed.__class__.__name__}: "
                                    f"{retry_failed}"
                                ),
                            )
                        logger.warning(
