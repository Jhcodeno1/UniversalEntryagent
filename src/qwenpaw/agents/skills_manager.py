...
def _get_allowed_builtin_skill_names() -> set[str] | None:
    """Return project-level builtin skill allowlist.

    ``None`` means keep the upstream QwenPaw behavior and allow all packaged
    builtins.  A comma/semicolon/whitespace separated env value limits the
    local pool to the listed builtin skills.
    """
    raw = os.environ.get("QWENPAW_SKILL_POOL_BUILTINS", "").strip()
    if not raw or raw == "*":
        return None
    names = {
        item.strip()
...
def _filter_allowed_builtin_versions(
    versions: dict[str, str],
) -> dict[str, str]:
    allowed = _get_allowed_builtin_skill_names()
    if allowed is None:
        return versions
    return {
        name: version
...
def list_builtin_import_candidates() -> list[dict[str, Any]]:
    ...
    builtin_dir = get_builtin_skills_dir()
    builtin_versions = _filter_allowed_builtin_versions(
...
def prune_disallowed_builtin_skills() -> list[str]:
    """Remove builtin pool entries outside the project allowlist.

    Customized skills are preserved even if their name is not allowlisted.
    """
    allowed = _get_allowed_builtin_skill_names()
    if allowed is None:
        return []
    ...
    def _process(payload: dict[str, Any]) -> None:
        skills = payload.setdefault("skills", {})
        payload["builtin_skill_names"] = sorted(allowed)
        for skill_name, entry in list(skills.items()):
...
def ensure_skill_pool_initialized() -> bool:
    ...
    if allowed is not None:
        import_builtin_skills(skill_names=sorted(allowed))
        prune_disallowed_builtin_skills()
    elif created:
...
class SkillService:
    ...
    def create_skill(
        ...
        def _update(payload: dict[str, Any]) -> None:
            ...
            if "source" in entry:
                source = entry["source"]
            elif skill_name in _filter_allowed_builtin_versions(
                _get_packaged_builtin_versions(),
            ):
                source = "builtin"
            else:
